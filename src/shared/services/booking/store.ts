...
type BookingState = {
  ...
  // 预约入口
  createBooking: (input: Omit<Booking, "id" | "code" | "status" | "createdAt">) => { ok: boolean; msg: string; booking?: Booking }
  // 核销
...
export const useBookingStore = create<BookingState>((set, get) => ({
  bookings: [
    { id: "bk1", courtyardId: "1", courtyardName: "木府", userId: "u_c_001", userName: "张小游", userPhone: "138****1001", date: "2026-06-30", slot: "09:00-10:00", visitors: 2, code: "YY280601", status: "pending", createdAt: "2026-06-28 10:00" },
    { id: "bk2", courtyardId: "3", courtyardName: "纳西古乐会", userId: "u_c_001", userName: "张小游", userPhone: "138****1001", date: "2026-06-15", slot: "20:00-21:30", visitors: 1, code: "YY150615", status: "checked", createdAt: "2026-06-10 14:00", checkedAt: "2026-06-15 20:05" },
  ],

  getBookingsByUser: (userId) => get().bookings.filter((b) => b.userId === userId),
  getBooking: (id) => get().bookings.find((b) => b.id === id),
  getBookingsByCourtyard: (courtyardId, date) => get().bookings.filter((b) => b.courtyardId === courtyardId && b.date === date && b.status !== "cancelled"),
  ...
  checkByCode: (code) => {
    ...
    set((s) => ({ bookings: s.bookings.map((b) => b.id === booking.id ? { ...b, status: "checked", checkedAt: new Date().toLocaleString("zh-CN") } : b) }))
