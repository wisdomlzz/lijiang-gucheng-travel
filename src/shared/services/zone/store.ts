...
const SEED: Zone[] = [
  { id: "zone_core", name: "古城核心区", stations: [
  ...
  { id: "zone_south", name: "古城南门", stations: [
  ...
  { id: "zone_inn", name: "客栈片区", stations: [
  ...
  { id: "zone_outskirt", name: "景区外围", stations: [
...
export const useZoneStore = create<ZoneState>((set, get) => ({
  zones: SEED,
  getStationsByType: (serviceType) => get().zones.flatMap((z) => z.stations.filter((s) => s.serviceType === serviceType)),
  getZoneByAddress: (address) => get().zones.find((z) => z.stations.some((s) => address.includes(s.address.slice(0, 3)))) ?? get().zones[0],
  addZone: (zone) => set((s) => ({ zones: [...s.zones, zone] })),
  removeZone: (zoneId) => set((s) => ({ zones: s.zones.filter((z) => z.id !== zoneId) })),
  updateZone: (zoneId, patch) => set((s) => ({ zones: s.zones.map((z) => z.id === zoneId ? { ...z, ...patch } : z) })),
  addStation: (zoneId, station) => set((s) => ({ zones: s.zones.map((z) => z.id === zoneId ? { ...z, stations: [...z.stations, station] } : z) })),
  removeStation: (zoneId, stationId) => set((s) => ({ zones: s.zones.map((z) => z.id === zoneId ? { ...z, stations: z.stations.filter((st) => st.id !== stationId) } : z) })),
}))

export type { Zone as ZoneModel } from "./store"
