// 遗产知识类型
export type HeritageType = 'road' | 'water' | 'well' | 'bridge' | 'ancient-tree' | 'protected-house' | 'historic-building' | 'human-environment';
...
export const heritageExtraFields: Record<HeritageType, { key: string; label: string }[]> = {
  ...
  water: [
    { key: 'flowDirection', label: '流向' },
  ],
  well: [
    { key: 'eyeCount', label: '眼数' },
  ],
  bridge: [
