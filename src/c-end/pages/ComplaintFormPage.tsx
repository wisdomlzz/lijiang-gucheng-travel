...
export function ComplaintFormPage() {
  ...
  const setField = (key: string) => setErrors((prev) => {
    const next = { ...prev };
    delete next[key];
    return next;
  });
    ...
    <div className="min-h-screen bg-surface-page pb-8">
      <div className="relative h-[132px] overflow-hidden">
        ...
        <div className="relative z-10 h-12 flex items-center px-4">
          <button onClick={() => navigate(-1)} className="size-9 -ml-2 rounded-full flex items-center justify-center active:scale-95">
      ...
      <div className="-mt-4 relative z-10 px-3 space-y-4">
        <section className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="mt-4 divide-y divide-slate-100">
            <InputRow required label="上报人" error={errors.reporterName}>
              <input value={reporterName} onChange={(e) => { setReporterName(e.target.value); setField("reporterName"); }} placeholder="请输入上报人姓名" className="field-input" />
            </InputRow>
            <InputRow required label="性别" error={errors.gender}>
              <div className="flex gap-8 text-[14px] text-text-body">
                {(["男", "女"] as const).map((g) => (
                  <button key={g} onClick={() => { setGender(g); setField("gender"); }} className="flex items-center gap-2">
                    <span className={`size-4 rounded-full border flex items-center justify-center ${gender === g ? "border-primary" : "border-slate-300"}`}>
            ...
            <InputRow required label="联系电话" error={errors.phone}>
              <input value={phone} maxLength={11} onChange={(e) => { setPhone(e.target.value.replace(/\D/g, "")); setField("phone"); }} placeholder="请输入联系电话" className="field-input" />
            </InputRow>
          </div>
        </section>
        ...
        <section className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="mb-4">
            <SelectRow required label="对象类型" value={objectType} placeholder="请选择对象类型" options={objectTypes} error={errors.objectType} onChange={(v) => { setObjectType(v); setField("objectType"); }} />
          </div>
          <div className="divide-y divide-slate-100">
            <InputRow required label="当事对象" error={errors.targetName}>
              <input value={targetName} onChange={(e) => { setTargetName(e.target.value); setField("targetName"); }} placeholder="请输入当事对象名称" className="field-input" />
            </InputRow>
            <SelectRow required label="事发片区" value={area} placeholder="请选择" options={areas} error={errors.area} onChange={(v) => { setArea(v); setField("area"); }} />
            <SelectRow required label="事发地点" value={location} placeholder="请选择" options={locations} error={errors.location} onChange={(v) => { setLocation(v); setField("location"); }} />
            <InputRow required label="门牌号" error={errors.doorplate}>
              <input value={doorplate} onChange={(e) => { setDoorplate(e.target.value); setField("doorplate"); }} placeholder="请输入门牌号" className="field-input" />
            </InputRow>
            <SelectRow required label="问题类型" value={issueType} placeholder="请选择问题类型" options={issueTypes} error={errors.issueType} onChange={(v) => { setIssueType(v); setField("issueType"); }} />
            <InputRow required label="反映内容" alignTop error={errors.content}>
              <textarea
                value={content}
                onChange={(e) => { setContent(e.target.value); setField("content"); }}
                placeholder="请输入反映内容"
                rows={5}
                ...
                <div className="flex gap-2">
                  {images.map((img, index) => (
                    <div key={img} className="relative size-16 rounded-xl overflow-hidden bg-slate-100">
                      ...
                      <button onClick={() => setImages(images.filter((_, i) => i !== index))} className="absolute right-1 top-1 size-5 rounded-full bg-black/55 text-white grid place-items-center">
                        <X size={12} />
                      </button>
                    </div>
                  ))}
                  {images.length < 3 && (
                    <button onClick={addImage} className="size-16 rounded-xl border border-dashed border-slate-300 bg-slate-50 text-slate-400 grid place-items-center">
                      <Camera size={22} />
                    </button>
                  )}
                </div>
        ...
        {complaintPhone && (
          <div className="rounded-2xl bg-white p-3 flex items-center gap-2 text-[12px] text-text-tertiary shadow-sm">
            ...
            紧急问题可拨打平台投诉电话
            <a href={`tel:${complaintPhone}`} className="text-primary font-medium">{complaintPhone}</a>
          </div>
        )}

        <button onClick={submit} className="w-full h-12 rounded-full bg-primary text-white text-[15px] font-medium active:scale-[0.98] transition-transform">
          提交投诉
        </button>
      </div>
    </div>
