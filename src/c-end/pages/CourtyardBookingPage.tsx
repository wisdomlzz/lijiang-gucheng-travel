...
import { useBookingStore, BOOKING_SLOTS } from "../../features/booking/store"
...
export function CourtyardBookingPage() {
  ...
  const handleBooking = () => {
    const result = createBooking({
      courtyardId: courtyard.id, courtyardName: courtyard.name,
      userId, userName, userPhone: "138****1001",
      date, slot, visitors,
    })
      ...
      <div className="min-h-full bg-surface-page">
        ...
        <div className="px-4 pt-8">
          <div className="bg-white rounded-2xl p-6 text-center">
            ...
            <p className="text-[13px] text-text-body mt-1">{date} · {slot} · {visitors}人</p>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => navigate("/c/my-bookings")} className="flex-1 h-10 rounded-xl bg-primary text-white text-[13px] font-medium">查看我的预约</button>
              <button onClick={() => navigate("/c/courtyards")} className="flex-1 h-10 rounded-xl bg-gray-100 text-text-body text-[13px]">返回院落</button>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-full bg-surface-page pb-24">
      ...
      <div className="px-4 pt-4 space-y-3">
        ...
        <div className="bg-white rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-text-tertiary">{courtyard.location} · {courtyard.hours}</p>
          </div>
        </div>

        {/* 日期选择 */}
        <div className="bg-white rounded-2xl p-4">
          <p className="text-[13px] font-semibold text-text-heading mb-3 flex items-center gap-1.5"><Calendar size={15} /> 选择日期</p>
            ...
            {days.map((d) => {
              ...
              return (
                <button key={d} onClick={() => setDate(d)}
                  ...
                  <span className="text-[10px] opacity-80">{isToday ? "今天" : new Date(d).toLocaleDateString("zh-CN", { weekday: "short" })}</span>
        ...
        {/* 时段选择 */}
        <div className="bg-white rounded-2xl p-4">
          <p className="text-[13px] font-semibold text-text-heading mb-3 flex items-center gap-1.5"><Clock size={15} /> 选择时段</p>
          <div className="grid grid-cols-3 gap-2">
            {BOOKING_SLOTS.map((s) => {
              const booked = getBookedByCourtyard(courtyard.id, date).filter((b) => b.slot === s).reduce((sum, b) => sum + b.visitors, 0)
              ...
              return (
                <button key={s} onClick={() => !full && setSlot(s)} disabled={full}
                  className={`h-11 rounded-lg text-[12px] font-medium transition ${
                    full ? "bg-gray-50 text-text-quaternary" : slot === s ? "bg-primary text-white" : "bg-gray-50 text-text-body"
                  }`}>
                  <div>{s}</div>
        ...
        {/* 参观人数 */}
        <div className="bg-white rounded-2xl p-4">
          <p className="text-[13px] font-semibold text-text-heading mb-3 flex items-center gap-1.5"><Users size={15} /> 参观人数</p>
          <div className="flex items-center gap-3">
            <button onClick={() => setVisitors((v) => Math.max(1, v - 1))} className="w-9 h-9 rounded-full bg-gray-100 text-text-body text-[18px]">-</button>
            <span className="text-[20px] font-bold w-10 text-center">{visitors}</span>
            <button onClick={() => setVisitors((v) => Math.min(10, v + 1))} className="w-9 h-9 rounded-full bg-gray-100 text-text-body text-[18px]">+</button>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-[390px] bg-white border-t border-gray-100 p-3">
        <button onClick={handleBooking} className="w-full h-12 rounded-xl bg-primary text-white text-[15px] font-medium active:scale-95 transition">
          确认预约
        </button>
      </div>
    </div>
