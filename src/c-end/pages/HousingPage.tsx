...
const realHouses = [
  {
    id: 1, name: "光义街9号公房", addr: "丽江市古城区光义街9号（铺面）",
    status: "rented", statusText: "出租", area: "gucheng", areaName: "古城区",
    hasPano: true, meta: ["267.74㎡", "特色食品", "砖木结构"],
    photos: [
  ...
  {
    id: 2, name: "新院巷63号公房", addr: "丽江市古城区新院巷63号",
    status: "idle", statusText: "未出租", area: "gucheng", areaName: "古城区",
    hasPano: true, meta: ["185.50㎡", "闲置", "木结构"],
    photos: [
  ...
  {
    id: 3, name: "五一街32号公房", addr: "丽江市古城区五一街32号",
    status: "rented", statusText: "出租", area: "gucheng", areaName: "古城区",
    hasPano: false, meta: ["156.30㎡", "饰品", "砖混结构"],
    photos: [
  ...
  {
    id: 4, name: "兴文巷18号公房", addr: "丽江市古城区兴文巷18号",
    status: "rented", statusText: "出租", area: "gucheng", areaName: "古城区",
    hasPano: true, meta: ["320.00㎡", "餐饮", "砖木结构"],
    photos: [
  ...
  {
    id: 5, name: "七一街8号公房", addr: "丽江市古城区七一街8号",
    status: "idle", statusText: "未出租", area: "gucheng", areaName: "古城区",
    hasPano: false, meta: ["98.60㎡", "房屋已收回", "砖混结构"],
    photos: [
      { bg: "linear-gradient(135deg,#E6F1FB 0%,#B5D4F4 100%)" },
      { bg: "linear-gradient(135deg,#B5D4F4 0%,#85B7EB 100%)" },
    ],
...
for (let i = 0; i < 50; i++) {
  ...
  houses.push({
    ...
    basic: [
      ...
      { label: "地图坐标", value: `${(26.86 + Math.random() * 0.04).toFixed(5)},${(100.23 + Math.random() * 0.02).toFixed(6)}` },
    ...
    contract: [
      ...
      { label: "合同状态", value: contractStatus, badge: contractStatus === "已签" ? "signed" : (contractStatus === "过期" ? "expired" : "unsigned") },
      { label: "商铺名称", value: isRented ? randPick(shopNames) : "——" },
    ],
    lease: [
      { label: "应缴金额(元)", value: leaseAmt, cls: isRented ? "price" : "" },
      { label: "缴费状态", value: payStatus, badge: payStatus === "已缴纳" ? "paid" : (payStatus === "未缴纳" ? "unpaid" : "") },
...
export function HousingPage() {
  ...
  const togglePano = () => { if (panoPlaying) stopPano(); else startPano(); };
  const prevPhoto = () => { if (!currentHouse) return; setCurrentPhoto(p => Math.max(0, p - 1)); };
  const nextPhoto = () => { if (!currentHouse) return; setCurrentPhoto(p => Math.min(currentHouse.photos.length - 1, p + 1)); };
          ...
          <div className="relative bg-black/5">
            <div className="overflow-hidden h-[200px]">
              <div
                ...
                style={{ width: `${currentHouse.photos.length * 100}%`, transform: `translateX(-${currentPhoto * (100 / currentHouse.photos.length)}%)` }}
              >
                {currentHouse.photos.map((p, i) => (
                  <div key={i} className="h-full flex-shrink-0 flex items-center justify-center" style={{ background: p.bg }}>
            ...
            {currentHouse.photos.length > 1 && (
              <>
                <button onClick={prevPhoto} className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center active:scale-95 transition-transform">
                  <ChevronLeft size={18} />
                </button>
                <button onClick={nextPhoto} className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center active:scale-95 transition-transform">
                  <ChevronRight size={18} />
                </button>
              </>
            )}
            <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between">
              <span className="text-[11px] text-white bg-black/40 px-2 py-0.5 rounded-full">{currentPhoto + 1} / {currentHouse.photos.length}</span>
              <div className="flex gap-1.5">
                {currentHouse.photos.map((_, i) => (
                  <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i === currentPhoto ? "bg-white w-4" : "bg-white/50"}`} />
                ))}
              </div>
            </div>
            {currentHouse.hasPano && (
              <button onClick={() => setShowPano(true)} className="absolute top-3 right-3 bg-white/90 rounded-lg px-3 py-1.5 text-[12px] text-primary font-medium flex items-center gap-1 shadow">
                <Play size={12} /> 全景
              </button>
            )}
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              ...
              <div className="px-4 py-3 bg-primary/5 flex gap-4">
                {currentHouse.meta.map((m, i) => (
                  <span key={i} className="text-[12px] text-primary/80 bg-white px-2.5 py-1 rounded-full">{m}</span>
                ))}
              </div>
            </div>
          </div>
              ...
              <div key={idx} className="bg-white rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="px-4 py-1">
                  {section.rows.map((row, i) => (
                    <div key={i} className={`flex items-start justify-between py-3 ${i !== section.rows.length - 1 ? "border-b border-[#F9F9F9]" : ""}`}>
                      <span className="text-[13px] text-text-secondary w-28 flex-shrink-0">{row.label}</span>
                      <span className={`text-[13px] text-text-body text-right ${row.cls === "price" ? "text-primary font-medium" : ""}`}>
                        {row.badge ? (
                          <span className={`text-[11px] px-2 py-0.5 rounded-full ${badgeClass[row.badge]}`}>{row.value}</span>
                        ) : row.value}
                      </span>
                    </div>
                  ))}
        ...
        {showPano && (
          <div className="fixed inset-0 bg-[#042C53] z-50 flex flex-col items-center justify-center gap-6">
            ...
            <div className="w-[320px] h-[210px] rounded-2xl bg-gradient-to-b from-[#042C53] via-[#185FA5] to-[#378ADD] flex flex-col items-center justify-center shadow-2xl">
              <button onClick={togglePano} className="w-14 h-14 rounded-full border-2 border-white/70 flex items-center justify-center active:scale-95 transition-transform">
                {panoPlaying ? <Pause size={22} className="text-white" /> : <Play size={22} className="text-white pl-0.5" />}
              </button>
            ...
            <div className="w-[280px] h-2 bg-white/15 rounded-full overflow-hidden">
              <div className="h-full bg-primary rounded-full transition-all duration-100" style={{ width: `${panoProgress}%` }} />
            </div>
            <p className="text-[#85B7EB] text-[13px]">{panoPlaying ? "播放中..." : (panoProgress >= 100 ? "播放完成" : "点击播放")}</p>
            <button onClick={() => { setShowPano(false); stopPano(); }} className="text-white text-[14px] px-8 py-2.5 rounded-full border border-white/30 bg-white/10">
              关闭全景
            </button>
          </div>
        )}
    ...
    <div className="min-h-full bg-surface-page flex flex-col">
      ...
      <div className="px-4 py-3 bg-white space-y-3">
        <div className="flex items-center gap-2 bg-[#F5F5F5] rounded-full h-11 px-4 focus-within:ring-2 focus-within:ring-primary/30 focus-within:bg-white transition-all">
          ...
          />
          <button onClick={() => {}} className="text-[12px] text-primary font-medium">筛选</button>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-1">
          {(["all", "rented", "idle"] as const).map(s => (
            <button
              key={s}
              onClick={() => { setCurrentFilter(s); setCurrentPage(1); }}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 pb-6">
        {displayed.map(house => (
          ...
          >
            <div className="relative h-[100px]" style={{ background: house.photos[0].bg }}>
              ...
              <span className="absolute bottom-2 left-3 text-[10px] text-white bg-black/40 px-2 py-0.5 rounded-full">{house.photos.length} 张照片</span>
              {house.hasPano && (
            ...
            <div className="p-3">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full ${badgeClass[house.status]}`}>{house.statusText}</span>
              </div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {house.meta.map((m, i) => (
                  <span key={i} className="text-[11px] text-text-secondary bg-surface-page px-2 py-0.5 rounded">{m}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="text-center pt-2">
            ...
            <p className="text-[11px] text-text-tertiary mt-2">已显示 {displayed.length} / {filtered.length} 条</p>
          </div>
        ) : (
