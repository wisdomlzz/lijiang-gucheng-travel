...
export function PointsCenterPage() {
  ...
  const account = useMemo(() => accounts[userId] ?? { userId, balance: 0, totalEarned: 0, totalUsed: 0 }, [accounts, userId])
  const ledgers = useMemo(() => allLedgers.filter((l) => l.userId === userId), [allLedgers, userId])

  // 按来源汇总
  const sourceSummary = ledgers.reduce((acc, l) => {
  ...
  return (
    <div className="min-h-full bg-gradient-to-b from-primary to-primary/80 pb-6">
      <PageHeader title="积分中心" back="/c/profile" theme="light" />
      ...
      <div className="px-4 mt-4 grid grid-cols-2 gap-3">
        <button onClick={simulateMallEarn} className="bg-white rounded-2xl p-4 flex flex-col items-center gap-2 active:scale-95 transition">
        ...
        <button onClick={goRedeem} className="bg-white rounded-2xl p-4 flex flex-col items-center gap-2 active:scale-95 transition">
          <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center">
            <Gift size={20} className="text-amber-500" />
          </div>
          <span className="text-[13px] font-medium text-text-heading">积分兑换</span>
          <span className="text-[11px] text-text-tertiary">跳转商城 <ExternalLink size={10} className="inline" /></span>
        </button>
      </div>
          ...
          <div className="divide-y divide-gray-50">
            {ledgers.map((l) => (
              <div key={l.id} className="flex items-center gap-3 px-4 py-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${l.direction === "IN" ? "bg-emerald-50" : "bg-orange-50"}`}>
                  {l.direction === "IN" ? <TrendingUp size={16} className="text-emerald-500" /> : <TrendingDown size={16} className="text-orange-500" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-text-heading">{l.sourceLabel}</p>
                  <p className="text-[11px] text-text-tertiary">{l.createdAt}{l.refId ? ` · ${l.refId}` : ""}</p>
                </div>
                <span className={`text-[15px] font-bold ${l.direction === "IN" ? "text-emerald-500" : "text-orange-500"}`}>
                  {l.direction === "IN" ? "+" : "-"}{l.delta}
                </span>
              </div>
            ))}
          </div>
