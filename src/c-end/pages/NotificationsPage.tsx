...
const realtimeNotificationTemplates: RealtimeNotificationTemplate[] = [
  { type: "order", title: "便民服务进度更新", summary: "您的便民服务订单已更新，请查看服务进度", targetUrl: "/c/orders" },
  { type: "order", title: "服务人员已接单", summary: "服务人员已接单，稍后会按约定时间上门服务", targetUrl: "/c/orders" },
...
export function NotificationsPage() {
  ...
  const handleCardClick = (item: { id: string; targetUrl?: string; isRead: boolean; type: NotificationType }, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!item.isRead) {
      markAsRead(item.id);
    }
              ...
              <div className="p-4 space-y-2">
                ...
                return (
                  <div key={item.id} className="relative overflow-hidden rounded-xl">
                    ...
                    >
                      <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${config.color}20` }}>
                        <Icon size={18} style={{ color: config.color }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-0.5">
                          <h3 className={`text-[14px] text-text-body ${!item.isRead ? "font-medium" : "text-text-secondary"}`}>
                            {item.title}
                          </h3>
