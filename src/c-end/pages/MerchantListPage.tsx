...
const categories = [
  ...
  { key: "food" as const, label: "餐饮" },
  { key: "hotel" as const, label: "住宿" },
  { key: "bar" as const, label: "酒吧" },
  { key: "shopping" as const, label: "购物" },
];

const categoryLabel: Record<MerchantCategory, string> = {
  all: "全部", food: "餐饮", hotel: "住宿", bar: "酒吧", shopping: "购物",
};
...
export function MerchantListPage() {
  ...
  const isNearby = location.pathname.includes("/nearby");
  ...
  const searchFn = useCallback(
    (m: (typeof filteredMerchants)[0], q: string) => m.name.includes(q),
    [],
  );
            ...
            >
              <div className="relative h-32">
                ...
                <div className="absolute top-2 left-2 flex gap-1.5">
                  <span className="px-2 h-5 flex items-center rounded-full bg-white/85 backdrop-blur text-[10px] text-text-heading">
                    {getMerchantCategoryLabel(m.category)}
                  </span>
                </div>
                {m._distance != null && (
                  <div className="absolute top-2 right-2 flex items-center gap-0.5 px-2 h-5 rounded-full bg-black/45 backdrop-blur">
                    <Navigation size={10} className="text-white" />
                    <span className="text-white text-[10px]">{formatDistance(m._distance)}</span>
                  </div>
                )}
                <div className="absolute bottom-2 left-3 right-3 flex items-end justify-between">
                  <div className="min-w-0 pr-2">
                    <div className="flex items-center gap-1">
                      <span className="text-white text-[16px] truncate drop-shadow">{m.name}</span>
                      <span className="flex-shrink-0 text-[10px] px-1.5 py-0.5 rounded-full bg-white/85 text-[#F59E0B] font-medium">{m.creditScore}分</span>
                    </div>
                    <div className="flex items-center gap-1 mt-0.5">
                      <Star size={11} fill="#FFD24A" stroke="#FFD24A" />
                      <span className="text-white text-[11px]">{m.rating}</span>
                      <span className="text-white/80 text-[11px]">· {m.reviewCount} 评价</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3">
                <p className="text-[13px] text-text-secondary line-clamp-1">{m.description}</p>
                <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-border-light">
                  <span className="text-[11px] text-text-tertiary flex items-center gap-1 truncate">
                    <MapPin size={11} className="flex-shrink-0" />
                    {m.address}
                  </span>
                  <span className="text-[12px] text-primary flex items-center gap-0.5 flex-shrink-0">
                    更多信息
