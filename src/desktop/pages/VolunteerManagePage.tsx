...
import {
  Search, Plus, MapPin, Clock, AlertTriangle, Heart, Crosshair, Loader2,
  Phone, Building2, Image as ImageIcon, CheckCircle2, XCircle, RefreshCw, Users,
} from "lucide-react"
...
function LocationMapField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  useEffect(() => {
    ...
    map.on("click", async (e: L.LeafletMouseEvent) => {
      ...
      try {
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${e.latlng.lat}&lon=${e.latlng.lng}&zoom=18&accept-language=zh`)
        const data = await res.json()
        onChange(data.display_name || `${e.latlng.lat.toFixed(4)}, ${e.latlng.lng.toFixed(4)}`)
      } catch { onChange(`${e.latlng.lat.toFixed(4)}, ${e.latlng.lng.toFixed(4)}`) }
    })
    marker.on("dragend", async () => {
      const pos = marker.getLatLng()
      try {
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${pos.lat}&lon=${pos.lng}&zoom=18&accept-language=zh`)
        const data = await res.json()
        onChange(data.display_name || `${pos.lat.toFixed(4)}, ${pos.lng.toFixed(4)}`)
      } catch { onChange(`${pos.lat.toFixed(4)}, ${pos.lng.toFixed(4)}`) }
    })
    mapRef.current = map
  ...
  const handleSearch = async () => {
    ...
    setSearching(true)
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(search)}&limit=5&accept-language=zh`)
      const data: NominatimResult[] = await res.json(); setResults(data)
    } catch { setResults([]) }
    setSearching(false)
  }
  ...
  const handleSelectResult = (r: NominatimResult) => {
    ...
    onChange(r.display_name); setResults([])
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSearch()}
            placeholder="搜索地点名称…" className="pl-9 h-8 text-[12px] rounded-lg" />
        </div>
        <Button size="sm" className="h-8 text-[11px] rounded-lg" onClick={handleSearch} disabled={searching}>
          {searching ? <Loader2 size={13} className="animate-spin" /> : <Search size={13} />}
          搜索
        </Button>
        <button onClick={() => { if (!navigator.geolocation) return; navigator.geolocation.getCurrentPosition(p => { mapRef.current?.setView([p.coords.latitude, p.coords.longitude], 16); markerRef.current?.setLatLng([p.coords.latitude, p.coords.longitude]) }) }}
          className="size-8 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 shrink-0" title="定位">
...
function exportSignUpXlsx(activityId: string | null) {
  ...
  if (!act) { toast.error("活动不存在"); return }
  ...
  for (const s of su) {
    ...
    for (const dr of drs) {
      rows.push({
        ...
        "电话": v?.phone || "",
        "日期": dr.date,
        "状态": { pending: "待签到", checked_in: "已签到", checked_out: "已签退", no_show: "未参与", checkout_overdue: "待补签退" }[dr.status] || dr.status,
        "签到时间": dr.checkInTime || "",
  ...
  if (!rows.length) { toast.error("暂无报名数据"); return }
  ...
  ws["!cols"] = [
    ...
    { wch: 18 }, { wch: 18 }, { wch: 10 }, { wch: 8 }, { wch: 20 },
  ]
...
export function VolunteerManagePage() {
  ...
  const handleCreate = () => {
    if (!formTitle.trim() || !formDesc.trim() || !formLocation.trim() || !formDateStart || !formDateEnd || !formDeadline) {
      toast.error("请填写完整信息"); return
    }
    if (new Date(formDateEnd) <= new Date(formDateStart)) { toast.error("结束日期必须在开始日期之后"); return }
    if (new Date(formDeadline) > new Date(formDateStart)) { toast.error("报名截止时间必须在活动开始时间之前"); return }
    ...
    const id = addActivity({
      ...
      location: formLocation.trim(), startTime, endTime,
      timeMode: "multi", dailyStartTime: formTimeStart, dailyEndTime: formTimeEnd,
    ...
    if (res.ok) { toast.success("活动已创建并发布"); setCreateOpen(false); resetForm() }
  ...
  const handleResolve = () => {
    ...
    if (res.ok) toast.success(res.msg); else toast.error(res.msg)
    setResolveTarget(null); setResolveCheckIn(""); setResolveCheckOut(""); setResolveNote("")
  }
  ...
  const openEditActivity = (act: typeof actStats[0]) => {
    ...
    setEditFormDateStart(act.startTime.slice(0, 10)); setEditFormDateEnd(act.endTime.slice(0, 10))
    setEditFormTimeStart(act.dailyStartTime || "09:00"); setEditFormTimeEnd(act.dailyEndTime || "17:00")
  ...
  const handleEditSave = () => {
    ...
    editActivity(editActTarget, {
      ...
      startTime, endTime,
      timeMode: "multi", dailyStartTime: editFormTimeStart, dailyEndTime: editFormTimeEnd,
  ...
  const resetForm = () => {
    setFormTitle(""); setFormDesc(""); setFormLocation(""); setFormDateStart(""); setFormDateEnd("")
    setFormTimeStart("09:00"); setFormTimeEnd("17:00")
    ...
    <PageLayout title="志愿服务管理" description="管理志愿者注册信息和志愿服务活动">
      ...
      <div className="flex items-center gap-6 mb-5 border-b border-slate-200">
        {(["activities", "volunteers"] as const).map((t) => (
          <button
            key={t}
            onClick={() => { setTab(t); setSelectedAct(null); setSelectedVol(null) }}
                  ...
                  volunteerPagination.paginatedItems.map((v) => {
                    ...
                    return (
                      <TableRow key={v.id} className={`hover:bg-slate-50/60 transition-colors ${isPending ? "bg-amber-50/40" : ""}`}>
                        ...
                        <TableCell>
                          {v.credentialImages?.length ? (<span className="text-[12px] text-slate-500">{v.credentialImages.length}张</span>) : (<span className="text-[11px] text-slate-300">无</span>)}
                        </TableCell>
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between mb-2">
              <button onClick={() => setSelectedVol(null)}
              ...
              >
                ...
                <div className="space-y-2.5">
                  {vol.reviewHistory.map((r: VolunteerReviewRecord, i: number) => {
                    ...
                    const borderColor = isApproved ? "border-emerald-100 bg-emerald-50/40" : isRejected ? "border-red-100 bg-red-50/40" : "border-sky-100 bg-sky-50/40"
                      ...
                      <div key={i} className={`rounded-lg border px-4 py-3 ${borderColor}`}>
                        <div className="flex items-center justify-between mb-0.5">
                          <span className={`text-[12px] font-medium flex items-center gap-1 ${textColor}`}>
                            {isApproved ? <CheckCircle2 size={13} /> : isRejected ? <XCircle size={13} /> : <RefreshCw size={13} />}
                            {actionLabel}
                          </span>
            ...
            {isPending && (
              ...
              >
                <button onClick={() => handleApproveVol(vol.id)}
                ...
                <button onClick={() => { setRejectVolTarget(vol.id); setRejectVolReason("") }}
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-slate-100 overflow-hidden bg-white">
                <div className="flex items-center justify-between px-5 pt-4 pb-2">
                  ...
                  <button onClick={() => exportSignUpXlsx(selectedAct)}
                ...
                <Table>
                  ...
                  <TableBody>
                    {signUps.filter((s) => s.activityId === selectedAct).map((su) => {
                      ...
                      const drs = dailyRecords.filter((d) => d.signUpId === su.id).sort((a, b) => a.date.localeCompare(b.date))
                      return drs.map((dr, di) => {
                        ...
                        return (
                          <TableRow key={dr.id} className={`hover:bg-slate-50/60 ${isAbnormal && !isResolved ? "bg-amber-50/30" : ""} ${isResolved ? "opacity-80" : ""}`}>
                            <TableCell className="text-[13px] font-medium text-slate-700">
                              {v?.name || su.volunteerId}
                              {dr.isLate && dr.lateMinutes && <span className="ml-1 text-[10px] text-amber-600">延时{dr.lateMinutes}min</span>}
                              {isResolved && <span className="ml-1.5 text-[10px] text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded">补录</span>}
                            </TableCell>
                            <TableCell className="text-[12px] text-slate-500 font-mono">{v?.phone || "-"}</TableCell>
                            <TableCell className="text-[12px] text-slate-500">{dr.date}</TableCell>
                            <TableCell><StatusBadge status={dr.status} config={DAILY_STATUS_CONFIG} /></TableCell>
                            ...
                            <TableCell className="text-[12px] text-slate-600 font-medium">{dr.serviceHours != null ? `${dr.serviceHours}h` : "-"}</TableCell>
                            <TableCell className="text-[11px] text-slate-400">{dr.reviewNote || (dr.isManual ? dr.reviewNote || "" : "")}</TableCell>
                            <TableCell className="text-right">
                              {isAbnormal && !isResolved && (
                                <TextAction label="补录" color="#D97706" bg="#FFFBEB" onClick={() => {
  setResolveCheckIn(dr.dayStartTime.slice(0, 16))
  setResolveCheckOut(dr.dayEndTime.slice(0, 16))
  setResolveTarget(dr.id); setResolveNote("")
}} />
                              )}
                            </TableCell>
                          </TableRow>
                    ...
                    {signUps.filter((s) => s.activityId === selectedAct).length === 0 && (
                      <TableCell colSpan={9} className="text-center py-10 text-slate-300 text-[13px]">暂无报名</TableCell>
                    )}
                  </TableBody>
                </Table>
              </div>
                    ...
                    {actStats.length === 0 ? (
                      ...
                      activityPagination.paginatedItems.map((a) => (
                        <TableRow key={a.id} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          {(() => {
                            const sd = new Date(a.startTime), ed = new Date(a.endTime)
      ...
      <Dialog open={createOpen} onOpenChange={(v) => { setCreateOpen(v); if (!v) resetForm() }}>
        <DialogContent className="sm:max-w-lg rounded-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3 py-2">
            <div><Label className="text-[12px]">活动名称 <span className="text-red-400">*</span></Label><Input value={formTitle} onChange={(e) => setFormTitle(e.target.value)} placeholder="例如：古城环境清洁日" className="mt-1 rounded-lg" /></div>
            <div><Label className="text-[12px]">活动描述 <span className="text-red-400">*</span></Label><textarea value={formDesc} onChange={(e) => setFormDesc(e.target.value)} placeholder="描述活动内容..." rows={3} className="w-full mt-1 rounded-lg border border-slate-200 px-3 py-2 text-[13px] focus:outline-none focus:ring-2 focus:ring-[#059669]/20 focus:border-[#059669]/40 resize-y" /></div>
            <div><Label className="text-[12px]">活动地点 <span className="text-red-400">*</span></Label>
              <LocationMapField value={formLocation} onChange={setFormLocation} />
            </div>
            <div>
              <Label className="text-[12px]">活动日期 <span className="text-red-400">*</span></Label>
              <div className="grid grid-cols-2 gap-2 mt-1">
                <input type="date" value={formDateStart} onChange={(e) => setFormDateStart(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
                <input type="date" value={formDateEnd} onChange={(e) => setFormDateEnd(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
              </div>
            </div>
            <div>
              <Label className="text-[12px]">每日时段 <span className="text-red-400">*</span></Label>
              <div className="grid grid-cols-2 gap-2 mt-1">
                <input type="time" value={formTimeStart} onChange={(e) => setFormTimeStart(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
                <input type="time" value={formTimeEnd} onChange={(e) => setFormTimeEnd(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-[12px]">报名开始 <span className="text-slate-400">(可选)</span></Label><input type="date" value={formEnrollStart} onChange={(e) => setFormEnrollStart(e.target.value)} className="w-full mt-1 h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" /></div>
              <div><Label className="text-[12px]">报名截止 <span className="text-red-400">*</span></Label><input type="date" value={formDeadline} onChange={(e) => setFormDeadline(e.target.value)} className="w-full mt-1 h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" /></div>
            </div>
            <div><Label className="text-[12px]">人数上限 <span className="text-red-400">*</span></Label><Input type="number" value={formMax} onChange={(e) => setFormMax(e.target.value)} min={1} className="mt-1 rounded-lg w-32" /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" size="sm" className="h-8 text-xs rounded-lg" onClick={() => setCreateOpen(false)}>取消</Button>
            <Button size="sm" className="h-8 text-xs rounded-lg bg-[#059669] hover:bg-[#047857]" onClick={handleCreate}>创建并发布</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editActTarget} onOpenChange={(v) => !v && setEditActTarget(null)}>
        <DialogContent className="sm:max-w-lg rounded-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3 py-2">
            <div><Label className="text-[12px]">活动名称 <span className="text-red-400">*</span></Label><Input value={editFormTitle} onChange={(e) => setEditFormTitle(e.target.value)} className="mt-1 rounded-lg" /></div>
            <div><Label className="text-[12px]">活动描述 <span className="text-red-400">*</span></Label><textarea value={editFormDesc} onChange={(e) => setEditFormDesc(e.target.value)} rows={3} className="w-full mt-1 rounded-lg border border-slate-200 px-3 py-2 text-[13px] focus:outline-none focus:ring-2 focus:ring-[#059669]/20 resize-y" /></div>
            <div><Label className="text-[12px]">活动地点 <span className="text-red-400">*</span></Label>
            ...
            <div>
              <Label className="text-[12px]">活动日期 <span className="text-red-400">*</span></Label>
              <div className="grid grid-cols-2 gap-2 mt-1">
                <input type="date" value={editFormDateStart} onChange={(e) => setEditFormDateStart(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
                <input type="date" value={editFormDateEnd} onChange={(e) => setEditFormDateEnd(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
              </div>
            </div>
            <div>
              <Label className="text-[12px]">每日时段 <span className="text-red-400">*</span></Label>
              <div className="grid grid-cols-2 gap-2 mt-1">
                <input type="time" value={editFormTimeStart} onChange={(e) => setEditFormTimeStart(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
                <input type="time" value={editFormTimeEnd} onChange={(e) => setEditFormTimeEnd(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
              </div>
            </div>
            <div>
              <Label className="text-[12px]">报名日期 <span className="text-red-400">*</span></Label>
              <div className="grid grid-cols-2 gap-2 mt-1">
                <input type="date" value={editFormEnrollStart} onChange={(e) => setEditFormEnrollStart(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
                <input type="date" value={editFormDeadline} onChange={(e) => setEditFormDeadline(e.target.value)} className="h-9 rounded-lg border border-slate-200 px-2.5 text-[12px] focus:outline-none" />
              </div>
            </div>
            <div><Label className="text-[12px]">人数上限 <span className="text-red-400">*</span></Label><Input type="number" value={editFormMax} onChange={(e) => setEditFormMax(e.target.value)} min={1} className="mt-1 rounded-lg w-32" /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!resolveTarget} onOpenChange={(v) => !v && setResolveTarget(null)}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader><DialogTitle className="text-[15px]">补录服务记录</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            {/* 根据异常类型显示不同上下文提示 */}
            ...
            <div>
              ...
              <Input type="datetime-local" value={resolveCheckIn} onChange={(e) => setResolveCheckIn(e.target.value)}
            ...
            <div>
              ...
              <Input type="datetime-local" value={resolveCheckOut} onChange={(e) => setResolveCheckOut(e.target.value)}
                className="mt-1 rounded-lg" />
            </div>
            ...
            {(() => {
              ...
              const ci = new Date(resolveCheckIn), co = new Date(resolveCheckOut)
            ...
            <div>
              <Label className="text-[12px]">补录备注 <span className="text-red-400">*</span></Label>
              <textarea value={resolveNote} onChange={(e) => setResolveNote(e.target.value)} rows={3} placeholder="请说明补录原因..."
          ...
          <DialogFooter>
            ...
            <Button size="sm" className="h-8 text-xs rounded-lg bg-[#2563EB] hover:bg-[#1D4ED8]" onClick={handleResolve}>确认补录</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!endActTarget} onOpenChange={(v) => !v && setEndActTarget(null)}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader><DialogTitle className="text-[15px] flex items-center gap-2">
            ...
            {(() => {
                  ...
                  <>
                    ...
                    <ul className="space-y-1.5 text-[12px]">
                      <li className="flex items-center gap-2 text-amber-700 bg-amber-50 rounded-lg px-3 py-1.5">
                        <AlertTriangle size={12} />{pending.length} 条待签到记录 → 标记为未参与
                      </li>
                      <li className="flex items-center gap-2 text-amber-700 bg-amber-50 rounded-lg px-3 py-1.5">
                        <AlertTriangle size={12} />{checkedIn.length} 条已签到记录 → 标记为待补签退
                      </li>
              ...
              return (
                <>
                  ...
                  {suCount > 0 && <p className="text-[12px] text-amber-700 bg-amber-50 rounded-lg px-3 py-2">{suCount} 人已报名，取消后报名记录保留。</p>}
                </>
              )
            })()}
          </div>
          <DialogFooter>
            <Button variant="outline" size="sm" className="h-8 text-xs rounded-lg" onClick={() => setEndActTarget(null)}>取消</Button>
            <Button size="sm" className="h-8 text-xs rounded-lg bg-red-500 hover:bg-red-600" onClick={handleEndActivity}>确认结束</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PageLayout>
