...
const mockPosts: Post[] = [
  ...
  },
  {
    ...
    status: "rejected",
    rejectReason: "内容涉及商业推广，请修改后重新提交",
    views: 0,
    likes: 0,
...
const tabs: { key: Tab; label: string; count: number }[] = [{ key: "info", label: "便民信息", count: 2 }]
                ...
                <div key={post.id} className="bg-white rounded-xl p-3">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[11px] px-1.5 py-0.5 rounded-full ${s.bg} ${s.color}`}>{s.label}</span>
                    </div>
                    ...
                    <div className="flex gap-2">
                      {post.status === "rejected" && (
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-1 rounded-full bg-primary-50 text-primary text-[11px]"
                        >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1 rounded-full bg-gray-100 text-text-secondary text-[11px]"
                      >
                        <Trash2 size={11} /> 删除
                      </button>
                    </div>
