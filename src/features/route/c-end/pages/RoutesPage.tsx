...
export function RoutesPage() {
  ...
  const filtered = keyword.trim()
    ? guides.filter((r) => r.name.includes(keyword.trim()) || r.spotNames.some((n) => n.includes(keyword.trim())))
    : guides
  ...
  return (
    <div className="min-h-screen bg-surface-page pb-6">
      ...
      <div className="px-3 space-y-3">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-[13px] text-text-tertiary">没有找到相关路线</div>
        ) : (
          visible.map((r) => (
            <button
              key={r.id}
            ...
            >
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0 py-1 flex flex-col">
                  <p className="text-[15px] text-text-body leading-snug line-clamp-2">{r.name}</p>
                  ...
                  <p className="mt-auto pt-2 text-[11px] text-text-tertiary line-clamp-1">{r.spotNames.join("、")}</p>
                </div>
              </div>
            </button>
          ))
