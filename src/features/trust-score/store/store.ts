...
type TrustScoreState = {
  ...
  getAllSupplierRatings: () => [
    string,
    SupplierRatingData & { avgRating: number; goodRate: number; creditScore: number },
  ][]
}
...
export const useTrustScoreStore = create<TrustScoreState>((set, get) => ({
        ...
        sc.staffId !== staffId
          ...
          : {
              ...
              trustScore: Math.min(
                100,
                sc.trustScore +
                  (rating >= 5 ? TRUST_SCORE_CONFIG.bonusRating5 : rating >= 4 ? TRUST_SCORE_CONFIG.bonusRating4 : 0)
              ),
              totalRatings: sc.totalRatings + 1,
              [`rating${rating}Count`]: (sc as any)[`rating${rating}Count`] + 1,
              scoreHistory: [
                {
                  id: `h${Date.now()}`,
                  date: nowStr(),
  ...
  addComplaintDeduction: (staffId, level) =>
    set((s) => ({
      scores: s.scores.map((sc) =>
        sc.staffId !== staffId
          ? sc
          : (() => {
              ...
              return {
                ...
                observationStartAt: newScore < 60 && sc.status !== "观察期" ? nowStr() : sc.observationStartAt,
                scoreHistory: [
      ...
      return {
        scores: s.scores.map((x) =>
          x.staffId === staffId
            ? {
                ...x,
                trustScore: Math.max(0, Math.min(100, x.trustScore + clamped)),
                scoreHistory: [
                  { id: `h${Date.now()}`, date: nowStr(), change: clamped, reason: `管理员调整: ${reason}` },
                  ...(x.scoreHistory || []),
                ],
              }
            : x
        ...
        adjustRecords: [
          ...s.adjustRecords,
          {
            ...
            afterScore: sc.trustScore + clamped,
            reason,
            createdAt: nowStr(),
          },
        ],
  ...
  getAllSupplierRatings: () =>
    Object.entries(get().supplierRatings).map(([id, data]) => [id, { ...data, ...computeSupplierRating(data) }]),
}))
