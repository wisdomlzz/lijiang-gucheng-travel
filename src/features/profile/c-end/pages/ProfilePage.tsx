...
export function ProfilePage() {
  ...
  const pendingServiceOrders = useMemo(
    () =>
      allOrders.filter((o) => !userId || o.userId === userId).filter((o) => o.status !== "S40" && o.status !== "S50")
        .length,
    [allOrders, userId]
  )
    ...
    <div className="min-h-full bg-surface-page">
        ...
        <div className="relative mt-2 flex items-center gap-4">
          <div className="relative">
            <div className="w-[70px] h-[70px] rounded-full overflow-hidden ring-2 ring-white/50 shadow-lg">
              <ImageWithFallback src={userAvatar} alt="avatar" className="w-full h-full object-cover" />
            </div>
      ...
      <div className="relative -mt-14 pb-8">
        ...
        <div className="px-4 mb-3">
          <button
            onClick={() => navigate("/c/points")}
            className="w-full bg-gradient-to-r from-amber-400 to-orange-500 rounded-2xl p-4 flex items-center justify-between active:scale-[0.98] transition-transform shadow-[0_4px_16px_rgba(245,158,11,0.25)]"
          >
        ...
        {/* 志愿服务统计（仅已认证志愿者显示） */}
        {volStats && (
          <button onClick={() => navigate("/c/volunteer/activities")} className="px-4 mb-3 block w-full">
