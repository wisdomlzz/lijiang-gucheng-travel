...
interface CreateActivityDialogProps {
  ...
  onFormChange: (key: keyof CreateActivityForm, value: string) => void
  onCreate: () => void
}

export function CreateActivityDialog({ open, onOpenChange, form, onFormChange, onCreate }: CreateActivityDialogProps) {
  return (
    <Dialog
      open={open}
      onOpenChange={(v) => {
    ...
    >
      <DialogContent className="sm:max-w-lg rounded-2xl max-h-[80vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button size="sm" className="h-8 text-xs rounded-lg bg-[#059669] hover:bg-[#047857]" onClick={onCreate}>
            创建并发布
          </Button>
        </DialogFooter>
      </DialogContent>
