    ...
    <div className="min-h-screen bg-surface-page">
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 left-0 right-0 flex items-center h-[52px] px-3">
          ...
          <span className="flex-1 text-center text-[17px] text-text-body font-medium">公告通知</span>{" "}
          <div className="w-9" />{" "}
        </div>{" "}
      </div>{" "}
      ...
      <div className="px-3 mt-3 space-y-3 pb-24">
        {" "}
        {visible.length === 0 ? (
          <div className="text-center py-12 text-text-secondary">{searchKeyword ? "未找到相关公告" : "暂无公告"}</div>
        ) : (
          <>
            {" "}
            {visible.map((ann) => (
              ...
              >
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0 py-0.5 flex flex-col">
                    {" "}
                    <p className="text-[14px] text-text-body leading-snug line-clamp-2">{ann.title}</p>{" "}
                    <p className="text-[12px] text-text-secondary mt-1 line-clamp-2">{ann.content.slice(0, 60)}...</p>{" "}
                    <p className="mt-auto text-[11px] text-text-tertiary">
            ...
            {hasMore && (
              <button onClick={loadMore} className="w-full py-3 text-center text-[14px] text-primary">
                加载更多
              </button>
            )}{" "}
          </>
