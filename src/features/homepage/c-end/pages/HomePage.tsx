...
export function HomePage() {
  ...
  const { visible: infoVisible, hasMore: infoHasMore, loadMore: infoLoadMore, reset: infoReset } = useLoadMore(allSorted, 5);

  useEffect(() => { infoReset() }, [infoReset]);
    ...
    <div className="min-h-full bg-surface-page pb-4">
      ...
      <div className="relative bg-gradient-to-b from-sky-mid via-sky-light to-surface-page pt-10 pb-12 px-4">
        ...
        <div className="flex items-center justify-center mb-4">
          <span className="text-white text-[18px] font-semibold tracking-[0.15em] drop-shadow-sm">
            丽江古城游
          </span>
        </div>
        ...
        {currentBanner && (
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <div className="text-white/90 text-[13px] tracking-[0.1em] mt-1 drop-shadow">
                一键服务 · 便捷生活
              </div>
            </div>
            {currentBanner.badge && (
              <div
                ...
                style={{ backgroundColor: currentBanner.badge === "NEW" ? "#10B981" : currentBanner.badge === "热门" ? "#EF4444" : "#3B82F6" }}
              >
                {currentBanner.badge}
              </div>
            )}
            ...
            {gridPages.map((_, i) => (
              <button
                ...
                className={`rounded-full transition-all cursor-pointer ${
                  i === gridPage
                    ? "w-6 h-1 bg-sky-deep"
                    : "w-3 h-1 bg-slate-300 hover:bg-slate-400"
                }`}
              />
            ))}
          </div>
