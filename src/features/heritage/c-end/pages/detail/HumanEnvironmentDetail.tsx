...
import { humanEnvironmentsData } from "@/features/heritage/shared/data/humanEnvironments"

export function HumanEnvironmentDetail() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const item = humanEnvironmentsData.find((h) => h.id === id) || humanEnvironmentsData[0]
  if (!item) {
    return (
