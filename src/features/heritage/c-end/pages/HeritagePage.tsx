...
import { HeritageType, HeritageItem, heritageTypeMeta } from "@/features/heritage/shared/types"
import { heritageData } from "@/features/heritage/shared/data/index"
...
export function HeritagePage() {
  ...
  // 根据tab筛选数据
  const getData = () => {
    let data = tab === "all" || tab === "nearby" ? Object.values(heritageData).flat() : heritageData[tab] || []

    if (keyword.trim()) {
      const lower = keyword.toLowerCase()
      data = data.filter((h) => h.name.toLowerCase().includes(lower) || h.description.toLowerCase().includes(lower))
    }
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[13px] whitespace-nowrap transition-colors ${
                tab === t.key ? "bg-primary text-white" : "bg-[#F5F5F5] text-text-secondary"
              }`}
            >
              {t.label}
            </button>
            ...
            return (
              ...
              >
                <div className="relative aspect-[16/9] overflow-hidden">
                  <ImageWithFallback src={item.photos?.[0]} alt={item.name} className="w-full h-full object-cover" />
                ...
                <div className="p-3">
                  <p className="text-[13px] text-text-secondary line-clamp-2 leading-relaxed">{item.description}</p>
                  {info.extra && (
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-[11px] text-text-tertiary">{info.extra}</span>
                      {distance !== null && (
                        <span className="text-[11px] text-primary font-medium">{formatDistance(distance)}</span>
                      )}
                    </div>
                  )}
                </div>
