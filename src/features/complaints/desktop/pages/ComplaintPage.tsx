...
export default function ComplaintPage() {
  ...
  const stats = useMemo(
    () => [
      ...
      {
        label: "已提交",
        value: fullList.filter((c) => c.status === "C10").length,
        color: "#D97706",
        bg: "#FEF3C7",
        icon: Clock,
      },
      {
        label: "已处理",
        value: fullList.filter((c) => c.status === "C40").length,
        color: "#059669",
        bg: "#D1FAE5",
        icon: BadgeCheck,
      },
      {
        label: "已驳回",
        value: fullList.filter((c) => c.status === "CR").length,
        color: "#DC2626",
        bg: "#FEE2E2",
        icon: XCircle,
      },
  ...
  const renderActions = (item: ComplaintWithOrder) => {
    ...
    return actions.map((act) => (
      <Button
        ...
        onClick={() => actionFn[act.action]?.(item.id)}
      >
        {act.label}
      </Button>
    ))
          ...
          {editPhone ? (
            <>
              ...
              <Button
                ...
                onClick={() => {
                  updateComplaintPhone(phoneValue)
                  toast.success("已保存")
                  setEditPhone(false)
                }}
              >
              ...
              <Button size="sm" variant="ghost" className="h-8 text-xs" onClick={() => setEditPhone(false)}>
                取消
              </Button>
            </>
          ) : (
              ...
              <Card className="rounded-xl border-slate-100 overflow-hidden">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-semibold tracking-tight mt-0.5" style={{ color: s.color }}>
                      {s.value}
                    </div>
                  </div>
                </CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                <TableHead className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">
                  投诉单号
                </TableHead>
                <TableHead className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">
                  上报人
                ...
                <TableHead className="text-[11px] font-medium text-slate-400 uppercase tracking-wider text-right">
                  操作
                </TableHead>
              </TableRow>
            </TableHeader>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl">
          {detail && (
            <>
              ...
              >
                {(() => {
                  const I = STATUS_META[detail.status].icon
                  return <I className="size-4" style={{ color: STATUS_META[detail.status].accent }} />
                })()}
                <span className="text-[13px] font-medium" style={{ color: STATUS_META[detail.status].accent }}>
                  {STATUS_META[detail.status].label}
                </span>
                {detail.handledAt && (
                  <span className="text-[11px] ml-auto opacity-60" style={{ color: STATUS_META[detail.status].accent }}>
                    {detail.handledAt}
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <InfoCard
                  icon={Building2}
                  title="当事对象信息"
                  lines={[`${detail.targetName || "—"}（${detail.objectType || "—"}）`, detail.type]}
                />
                <InfoCard
                  icon={MapPin}
                  title="事发位置"
                  lines={[`${detail.incidentArea || "—"} · ${detail.incidentLocation || "—"}`, detail.doorplate || "—"]}
                />
              </div>
                ...
                <div className="rounded-xl border border-slate-100 p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    {detail.images.map((img) => (
                      <div
                        key={img}
                        className="aspect-[4/3] rounded-lg overflow-hidden border border-slate-100 bg-slate-50"
                      >
                        <ImageWithFallback
                          src={img}
                          alt="投诉附件"
              ...
              {detail.result && (
                <div
                  className={`rounded-xl p-4 text-[13px] leading-relaxed border ${
                    detail.status === "CR"
              ...
              {detail.status !== "C10" && (
                <DialogFooter>
                  <Button variant="outline" className="h-8 text-xs rounded-lg" onClick={() => setDetailId(null)}>
                    关闭
                  </Button>
                </DialogFooter>
              )}
      ...
      >
        <DialogContent className="max-w-md rounded-2xl">
          ...
          <DialogFooter>
            ...
            <Button className="h-8 text-xs rounded-lg bg-[#059669] hover:bg-[#047857]" onClick={confirmResolve}>
              确认处理完成
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      >
        <DialogContent className="max-w-md rounded-2xl">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" className="h-8 text-xs rounded-lg" onClick={confirmReject}>
              确认驳回
            </Button>
          </DialogFooter>
        </DialogContent>
  ...
  return (
    <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-3.5">
      ...
      <div className="space-y-1">
        {lines.map((line) => (
          <p key={line} className="text-[12px] text-slate-500">
            {line}
          </p>
        ))}
      </div>
