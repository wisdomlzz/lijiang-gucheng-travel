...
export function MyComplaintsPage() {
  ...
  const tabFiltered = tab === "all" ? myComplaints
    : myComplaints.filter((c) => c.status === tab);
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0">
                          <div className="text-[14px] font-medium text-text-heading truncate">{c.targetName || "投诉反馈"}</div>
                          <div className="text-[11px] text-text-tertiary mt-0.5">{c.id}</div>
                        </div>
                        <span className={`shrink-0 ml-2 text-[11px] px-2 py-1 rounded-full flex items-center gap-1 ${cfg.bg} ${cfg.color}`}>
                          <Icon size={10} />
                          {cfg.label}
                        </span>
                      </div>
