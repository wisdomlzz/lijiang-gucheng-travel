  ...
  return (
    <div className="min-h-full bg-surface-page pb-24">
      ...
      <div className="p-3 space-y-3">
        ...
        <div className="bg-white rounded-2xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <input
              ...
              className="h-10 rounded-xl bg-surface-page px-3 text-[13px] outline-none"
            />
            <input
              value={form.city}
        ...
        >
          ...
          <span className={`w-11 h-6 rounded-full p-0.5 transition ${form.isDefault ? "bg-primary" : "bg-[#D1D5DB]"}`}>
            <span
              className={`block w-5 h-5 rounded-full bg-white transition ${form.isDefault ? "translate-x-5" : ""}`}
            />
          </span>
        </button>
      </div>
      ...
      <div className="fixed left-0 right-0 bottom-0 p-3 bg-gradient-to-t from-surface-page to-transparent pb-[calc(env(safe-area-inset-bottom)+12px)]">
        <button onClick={handleSave} className="w-full h-11 rounded-full bg-primary text-white text-[14px] shadow-lg">
          保存地址
        </button>
      </div>
    </div>
