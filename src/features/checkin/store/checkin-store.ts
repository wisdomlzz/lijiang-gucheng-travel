...
const COURTYARDS = [
  ...
  { id: "5",  name: "四方街",       lat: 26.8680, lng: 100.2345, address: "古城中心四方街",          w: 11 },
  ...
  { id: "9",  name: "周霖故居",     lat: 26.8690, lng: 100.2365, address: "七一街兴文巷",            w: 3 },
  { id: "10", name: "顾彼得旧居",   lat: 26.8682, lng: 100.2328, address: "光义街金星巷",            w: 3 },
  { id: "11", name: "洛克故居",     lat: 26.8678, lng: 100.2350, address: "五一街文生巷",            w: 2 },
  { id: "12", name: "净莲寺",       lat: 26.8705, lng: 100.2305, address: "狮子山净莲寺",            w: 4 },
  { id: "13", name: "普贤寺",       lat: 26.8672, lng: 100.2335, address: "七一街崇仁巷",            w: 2 },
  { id: "14", name: "白马龙潭寺",   lat: 26.8660, lng: 100.2260, address: "光义街白马龙潭寺",        w: 6 },
  ...
  { id: "18", name: "银饰坊",       lat: 26.8693, lng: 100.2340, address: "七一街银饰坊",            w: 3 },
  { id: "19", name: "东巴纸坊",     lat: 26.8670, lng: 100.2338, address: "七一街八一巷",            w: 6 },
  { id: "20", name: "和志刚书斋",   lat: 26.8696, lng: 100.2358, address: "五一街振兴巷",            w: 2 },
  { id: "21", name: "古城画院",     lat: 26.8686, lng: 100.2320, address: "光义街现文巷",            w: 8 },
  { id: "22", name: "忠义坊",       lat: 26.8676, lng: 100.2310, address: "木府前忠义坊",            w: 7 },
  { id: "23", name: "官门口",       lat: 26.8684, lng: 100.2330, address: "四方街官门口",            w: 3 },
  { id: "24", name: "大水车",       lat: 26.8720, lng: 100.2320, address: "古城北门大水车广场",      w: 7 },
  { id: "25", name: "纳西人家",     lat: 26.8697, lng: 100.2360, address: "五一街文明巷",            w: 3 },
]
const USERS = ["张三","李四","王五","赵六","孙七","周八","吴九","郑十","钱一","沈二","陈三","朱四","黄五","林六","何七","马八","高九","罗十","梁一","宋二","唐三","韩四","杨五","许六","曹七"]
const PHOTOS = [
...
function genSeeds(): Checkin[] {
  const r: Checkin[] = []; let idx = 0
  COURTYARDS.forEach((c) => {
    for (let i = 0; i < c.w; i++) {
      idx++; const month = idx <= 55 ? 5 : 6; const day = month === 5 ? 15 + (idx % 17) : 1 + ((idx - 56) % 28)
      const hour = 8 + ((idx * 3) % 12); const min = (idx * 17) % 60
      r.push({ id: `chk-${idx}`, courtyardId: c.id, courtyardName: c.name, userId: `user-${(idx * 3) % 30 + 1}`, userName: USERS[(idx * 7 + 3) % USERS.length], photo: PHOTOS[(idx * 5 + 1) % PHOTOS.length], location: { lat: c.lat + ((i * 3 + 1) % 7 - 3) * 0.0003, lng: c.lng + ((i * 5 + 2) % 7 - 3) * 0.0003 }, address: c.address, createdAt: `2026-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")} ${String(hour).padStart(2, "0")}:${String(min).padStart(2, "0")}`, status: "approved" })
    }
  })
  return r
}
...
export const useCheckinStore = create<CheckinState>((set, get) => ({
  ...
  addCheckin: (c) => {
    ...
    set((s) => ({ checkins: [{ ...c, id, createdAt: new Date().toLocaleString("zh-CN"), status: "approved" }, ...s.checkins] }))
  ...
  canCheckin: (userId, courtyardId) => {
    ...
    const existing = get().checkins.find((c) => c.userId === userId && c.courtyardId === courtyardId && new Date(c.createdAt.replace(/\//g, "-")).getTime() >= start && new Date(c.createdAt.replace(/\//g, "-")).getTime() < start + 86400000)
