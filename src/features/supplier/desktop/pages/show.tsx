    ...
    <PageLayout title="申请详情">
      <div className="max-w-2xl">
        ...
        >
          ...
          <StatusBadge status={STATUS_LABELS[app.status]} color={STATUS_COLORS[app.status]} />
        </div>
        ...
        <div className="bg-white rounded-xl p-6 mb-6">
          ...
          <div className="space-y-3">
            {infoRows.map((row) => {
              const Icon = row.icon
              return (
                <div key={row.label} className={`flex items-start gap-3 ${row.span ? "flex-col" : ""}`}>
                  <div className="flex items-center gap-2 w-28 flex-shrink-0">
                    <Icon size={14} className="text-text-tertiary" />
                    <span className="text-[13px] text-text-tertiary">{row.label}</span>
                  </div>
                  <span className={`text-[13px] text-text-body ${row.span ? "mt-1" : ""}`}>{row.value}</span>
                </div>
              )
            })}
          </div>
          ...
          <div className="mt-4 pt-4 border-t">
            ...
            <img src={app.licenseImg} alt="营业执照" className="w-full max-w-md rounded-xl object-cover h-48" />
          </div>
            ...
            {showRejectForm ? (
              <div className="space-y-3">
                ...
                <div className="flex gap-3">
                  <Button className="flex-1 bg-green-500 hover:bg-green-600" onClick={handleApprove}>
