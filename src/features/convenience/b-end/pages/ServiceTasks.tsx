...
import { Phone, MapPin, ChevronRight, AlertTriangle, Wallet, CheckCircle2 } from "lucide-react"
...
export function ServiceTasks() {
  ...
  const tabFiltered = orders.filter((o) =>
    tab === "已指派"
      ? o.state === "pending"
      : tab === "进行中"
        ? o.state !== "pending" && o.state !== "done" && o.state !== "cancelled" && o.state !== "manual"
        : tab === "已完成"
          ? o.state === "done"
          : o.state === "cancelled"
  )
    ...
    <div className="pb-4">
      ...
      {(manualCount > 0 || cancelReviewCount > 0) && (
        <div className="px-4 mt-2 space-y-1.5">
          {manualCount > 0 && (
            <button
              onClick={() => {
                const t = orders.find((o) => o.state === "manual")
                if (t) setOpenId(t.id)
              }}
          ...
          {cancelReviewCount > 0 && (
            <button
              onClick={() => {
                const t = orders.find((o) => cancelRequestedOrders.some((c) => c.id === o.id))
                if (t) setOpenId(t.id)
              }}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  >
                    {t.type.length > 12 ? t.type.slice(0, 12) + "…" : t.type}
                  </div>
                </div>
              ...
              <div
                ...
                className="mt-3 flex items-center gap-2 border-t border-[#F0F0F0] pt-2.5"
              >
                <button
                  onClick={() => {
                    setToast("拨打电话")
      ...
      {showDispatch && tab === "已指派" && nextPending && (
        <div className="absolute inset-0 z-30 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm">
          <div
            className="w-[340px] mx-auto bg-white rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.25)]"
            style={{
              background: urgent
                ? "linear-gradient(180deg, #FEE2E2 0%, #FFFFFF 35%)"
                : "linear-gradient(180deg, #FEF3C7 0%, #FFFFFF 35%)",
            }}
          >
            <div className="px-5 pt-5 pb-2">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  <div className="text-[14px] font-medium text-text-heading">
                    {urgent ? "派单即将转派！" : "新派单"}
                  </div>
                  <div className="text-[11px] text-text-tertiary">便民服务派单 · 5 分钟内未响应将自动转派</div>
                </div>
              </div>
            </div>
            <div className="px-5 py-2 text-center">
              <div
                className="inline-block px-4 py-2 rounded-2xl text-white text-[26px] font-medium tabular-nums"
                style={{ background: urgent ? "#DC2626" : "#F59E0B" }}
              >
                {fmt(counter)}
              </div>
            ...
            <div className="mx-5 my-3 bg-primary-50/60 rounded-2xl p-3 space-y-1.5 text-[12px] text-text-body">
              ...
              {nextPending.ref && (
                <div className="text-text-tertiary">
                  {nextPending.ref}
                  {nextPending.note ? ` · ${nextPending.note}` : ""}
                </div>
              )}
            </div>
