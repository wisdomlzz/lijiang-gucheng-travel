...
export function ServiceProfile() {
  ...
  const stats = useMemo(() => {
    ...
    const completionRate =
      terminalOrders.length > 0 ? `${Math.round((doneOrders.length / terminalOrders.length) * 100)}%` : "—"
    ...
    <div className="pb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-1.5">
              <span className="text-[16px] text-text-heading font-medium">李师傅</span>
        ...
        {storeTrustScore && (
          <div className="mt-4 bg-white/90 backdrop-blur-xl rounded-2xl p-3.5 shadow-[0_4px_16px_rgba(60,120,200,0.10)]">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-1 mt-0.5">
                  ...
                  <span
                    className={`ml-2 inline-flex items-center gap-0.5 text-[11px] ${
                      storeTrustScore.status === "正常" ? "text-[#059669]" : "text-[#DC2626]"
                    }`}
                  >
                    {storeTrustScore.status === "正常" ? (
            ...
            <div className="flex items-center justify-between mt-1.5 text-[10px] text-text-tertiary">
              ...
              <span>
                好评率{" "}
                {storeTrustScore.totalRatings > 0
                  ? Math.round(
                      ((storeTrustScore.rating5Count + storeTrustScore.rating4Count) / storeTrustScore.totalRatings) *
                        100
                    )
                  : 0}
                %
      ...
      <div className="px-4 mt-3">
        <div className="bg-white rounded-2xl shadow-[0_4px_16px_rgba(60,120,200,0.08)] overflow-hidden">
          ...
          />
          <Row icon={<MapPin />} tint="#10B981" label="服务片区" extra="大研 · 束河" onClick={() => {}} />
        </div>
      </div>
      ...
      <div className="px-4 mt-4 text-center text-[11px] text-text-tertiary">丽江古城游 B 端 · 服务人员版 · v2.0.0</div>
    </div>
  )
}

function Stat({ tint, label, value, star }: { tint: string; label: string; value: string; star?: boolean }) {
  return (
    <div className="bg-white rounded-2xl p-3 shadow-[0_4px_16px_rgba(60,120,200,0.08)]">
      ...
      <div className="mt-1 text-[15px] font-medium flex items-center gap-1" style={{ color: tint }}>
...
function Row({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 px-4 py-3 border-b border-[#F0F0F0] last:border-b-0 active:bg-primary-50/40"
    >
      <div
      ...
      <span className="text-[13px] text-text-body flex-1 text-left">{label}</span>
      {extra && <span className="text-[11px] text-text-tertiary">{extra}</span>}
