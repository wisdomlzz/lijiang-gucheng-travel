...
export function DetailLayout({
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-center px-3 pt-3 pb-2 shrink-0 z-10"
      ...
      >
        ...
        <span className="flex-1 text-center text-[15px] text-text-heading -ml-9">
          {title}
        </span>
      </div>
      <div className="flex-1 overflow-y-auto no-scrollbar">{children}</div>
      {footer && (
        <div className="bg-white border-t border-[#F0F0F0] px-4 py-3 shrink-0">
          {footer}
        </div>
      )}
    </div>
