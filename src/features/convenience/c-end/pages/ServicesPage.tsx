...
import { useAddressStore } from "../../../address/store/address-store"
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div className="flex-1 text-left">
                {selectedAddress ? (
                  ...
                  <p className="text-[13px] text-text-tertiary">
                    请选择{service.type === "grid" ? "服务" : "起点"}地址
                  </p>
                )}
              </div>
              ...
              >
                ...
                <div className="flex-1 text-left">
                  {selectedEndAddress ? (
                    <>
                      <p className="text-[13px] text-text-body">
                        {selectedEndAddress.name} {selectedEndAddress.phone}
                      </p>
                      <p className="text-[11px] text-text-tertiary truncate">
                        {selectedEndAddress.province}
                        {selectedEndAddress.city}
                        {selectedEndAddress.district} {selectedEndAddress.detail}
                      </p>
                    </>
                  ) : (
          ...
          <div>
            ...
            <textarea
              ...
              required
            />
            {getDescriptionHint() && <p className="text-[11px] text-text-tertiary mt-1">{getDescriptionHint()}</p>}
          </div>
          ...
          <div>
            ...
            {isLateNight && <p className="text-[11px] text-[#EF4444] mt-1">该时段仅支持预约明日7:00后的服务</p>}
                  ...
                  <div className="flex gap-2 overflow-x-auto">
                    {Array.from({ length: 7 }, (_, i) => {
                      ...
                      return (
                        ...
                        >
                          <p className={`text-[11px] ${isSelected ? "text-white/80" : "text-text-tertiary"}`}>
                            {dayStr}
                          </p>
                          <p className={`text-[13px] font-medium ${isSelected ? "text-white" : "text-text-body"}`}>
                            {dateStr}
                          </p>
                        </button>
                      )
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[
                        ...
                        const isEarly =
                          isLateNight &&
                          scheduleDate === `${new Date().getMonth()}-${new Date().getDate() + 1}` &&
                          hour < 7
                          ...
                          <button
                            ...
                            className={`py-2 rounded-lg text-[12px] ${
                              scheduleTime === t
                                ...
                                : "bg-white border border-border-light text-text-body"
                            }`}
                          >
                            {t}
                          </button>
          ...
          <div className="bg-gradient-to-r from-[#DBEAFE] to-[#BFDBFE] rounded-xl p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-text-heading whitespace-pre-line leading-relaxed">{service.priceNote}</p>
              </div>
            </div>
          </div>
                  ...
                  {addresses.map((addr) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {addr.isDefault && (
                          <span className="text-[10px] bg-primary-100 text-primary px-1.5 py-0.5 rounded">默认</span>
                        )}
                      </div>
...
export function ServicesPage() {
  ...
  const recentOrder = (() => {
    const userOrders = user
      ? orders.filter((o) => o.userId === user.id && o.status !== "S50")
      : orders.filter((o) => o.status !== "S50")
  ...
  return (
    <div className="min-h-full bg-gradient-to-b from-primary-100 via-primary-50 to-surface-page flex flex-col">
      <PageHeader title="便民服务" back={() => navigate(-1)} />
      ...
      <div className="flex-1 px-3 pb-4">
        <div className="mb-3">
          <h2 className="text-[14px] text-primary font-medium border-b-2 border-primary inline-block pb-1">便民服务</h2>
        </div>
          ...
          {services.map((service) => (
            ...
            >
              <div className="relative h-[99px] overflow-hidden">
                ...
                <div
                  className={`absolute top-2 left-2 ${service.tagColor} text-white text-[11px] px-2 py-1 rounded-full`}
                >
                  {service.tag}
                </div>
              ...
              <div className="px-2.5 pt-2.5 pb-2">
                <p className="text-[14px] text-text-body font-medium mb-0.5 text-left">{service.name}</p>
                <p className="text-[12px] text-primary font-medium text-left">{service.price}</p>
              </div>
            </button>
          ))}
        </div>
