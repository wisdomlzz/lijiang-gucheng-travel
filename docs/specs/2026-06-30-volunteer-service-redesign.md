...
现有志愿者模块存在以下缺口：
- 志愿者认证无审核流程，注册即生效
- 活动仅3态（draft/published/ended），缺少审核和细粒度流转
...
| reviewNote | string? | 审核意见（驳回原因） |
| reviewedAt | string? | 审核时间 |
| createdAt | string | |

### VolunteerActivity（5态）

```
draft → pending_review → published → in_progress → ended
  ↓          ↓              ↓            ↓
...
| `vol:act:{id}:settle` | endTime + 1min | `checked_in` → `checkout_overdue`，`signed_up` → `no_show` |

活动创建/审核通过时注册定时器，活动取消/删除时清除。
...
| 从 | 动作 | 到 | 条件 |
|---|---|---|---|
| draft | submit | pending_review | 必填字段完整 |
| draft | cancel | cancelled | |
| pending_review | approve | published | |
| pending_review | reject | draft | 必填 reviewNote |
...
| 页面 | 改动 |
|---|---|
| 志愿者管理 | 增加 status 筛选；待审核志愿者高亮；审核操作（通过/驳回+原因）|
| 活动管理 | 5态流转；审核操作；创建/编辑时增加 enrollStartTime 字段 |
| 活动详情 | 异常报名记录列表；管理员处理弹窗 |
